...
def test_stable_smooth_rejects_ndim_above_one():
    """Tensor (>=2D) inputs aren't claimed -- the DimShuffle whitelist used
    by the elementwise check is broadcast-safe for 0/1-D inputs but
    can't distinguish axis permutations from broadcasts at higher rank.
    Reject up front rather than risk silently miscomputing gradients."""
    from taylor_remainder import stable_smooth

    x = pt.dmatrix("x")
    with pytest.raises(NotImplementedError, match=r"only scalar .* vector"):
        stable_smooth(pt.sin(x), x, 0.0, denominator_degree=1)
